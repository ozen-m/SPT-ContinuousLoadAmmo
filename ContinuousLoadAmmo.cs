...
[BepInPlugin("com.ozen.continuousloadammo", "Continuous Load Ammo", "1.1.4")]
...
public class ContinuousLoadAmmo : BaseUnityPlugin
{
    ...
    public void Awake()
    {
        ...
        SpeedLimit = Config.Bind("General", "Speed Limit", 0.45f, new ConfigDescription("The speed limit, as a percentage of the walk speed, set to the player while loading ammo", new AcceptableValueRange<float>(0f, 1f), new ConfigurationManagerAttributes() { Order = 6, ShowRangeAsPercent = true }));
        ...
        MagPresetFallback = Config.Bind("General", "Mag Preset Fallback", true, new ConfigDescription("Fallback to normal quick load if magazine preset is unavailable", null, new ConfigurationManagerAttributes() { Order = 3 }));
        ...
        QuickLoadMode = Config.Bind("Quick Load", "Mode", Models.QuickLoadMode.LastMagazinePreset, new ConfigDescription("Highest Penetration Available - choose ammo that has the highest penetration power.\nLast Bullet in Magazine - prioritize the last ammo of the current weapon's magazine.\nLast Used Magazine Preset - load the last used magazine preset", null, new ConfigurationManagerAttributes() { Order = 1 }));
